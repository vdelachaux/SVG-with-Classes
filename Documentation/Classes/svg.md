...
|**.cursor**|Text|Current target reference (stored id or DOM reference) used by subsequent calls.|**""**|
...
|.**exportToBase64** ( {withDataPrefix : `Boolean`} ) → `Text` | Returns the SVG tree encoded in base64. If `withDataPrefix` is `True`, the result is prefixed with `data:image/svg+xml;base64,`.
...
|.**linearGradient** ( id : `Text` {; startColor : `Text` {; stopColor : `Text` {; options: `Object` }}}) → ref: `Text` | Sets a new linear gradient in the current canvas
|.**radialGradient** ( id : `Text` {; startColor : `Text` {; stopColor : `Text` {; options: `Object` }}}) → ref: `Text` | Sets a new radial gradient in the current canvas
|.**pattern** ( id : `Text` {; options: `Object` }) → ref: `Text` | Defines a new pattern container in the current canvas and returns its reference
|.**solidColor** ( id : `Text` ; color : `Text` {; opacity : `Real` } ) → `cs.svg` | Defines a `solidColor` paint server (a single color with opacity) that can be referenced like a gradient.
|.**symbol** ( id : `Text` {; applyTo } ) → `cs.svg` | To define a symbol
|.**marker** ( id : `Text` {; applyTo } {; options : `Object` } ) → `cs.svg` | Defines a `marker` (in the canvas defs) or, when applied to an element/position (`start`/`mid`/`end`), references an existing marker on it.
...
|.**import** ( file : `4D.File` {; applyTo } ) → `cs.svg` | Imports the content of an external SVG file into the current container.
|.**title** ( title : `Text` ) → `cs.svg` | Adds a `title` element to the document.
|.**desc** ( description : `Text` ) → `cs.svg` | Adds a `desc` (description) element to the document.
|.**comment** ( comment : `Text` {; attachTo } ) → `cs.svg` | Inserts an XML comment node.
...
|.**tspan** ( text : `Text` {; attachTo } ) → `cs.svg` | Creates a `tspan` element (styled text span) inside a `text` element.
...
|Function|Action|
|--------|------|
|.**setID** ( id : `Text` {; applyTo } ) → `cs.svg` | Sets the `id` attribute of the SVG object (standard XML attribute).<br>If this attribute already exists, its value is replaced by the value passed in parameter.
...
|.**rotateRad** ( radians : `Real` {; cx : `Real` ; cy : `Real`} {; applyTo } ) → `cs.svg` | Same as `.rotate()` but the angle is expressed in radians.
|.**rotateTAU** ( fraction : `Real` {; cx : `Real` ; cy : `Real`} {; applyTo } ) → `cs.svg` | Same as `.rotate()` but the angle is given as a fraction of a full turn (TAU): `0.25` = 90°, `0.5` = 180°…
|.**transform** ( node : `Text` ; transform : `Text` {; stack : `Boolean` } ) | Sets (or, if `stack` is `True`, appends to) the `transform` attribute of `node`.
...
|.**linecap** ( value : `Text` \| `Integer` {; applyTo } ) → `cs.svg` | Sets the `stroke-linecap` attribute (`butt`, `round`, `square`).
|.**linejoin** ( value : `Text` \| `Integer` {; applyTo } ) → `cs.svg` | Sets the `stroke-linejoin` attribute (`miter`, `round`, `bevel`).
...
|.**dx** ( value : `Real` \| `Text` {; applyTo } ) → `cs.svg` | Sets the `dx` attribute (horizontal shift) of a `text`/`tspan`.
|.**dy** ( value : `Real` \| `Text` {; applyTo } ) → `cs.svg` | Sets the `dy` attribute (vertical shift) of a `text`/`tspan`.
...
|.**Star** ( nBranches : `Integer` ; outerRadius : `Real` ; cx : `Real` ; cy : `Real` {; ratio : `Real` } ) → `cs.svg` | Draws a star with `nBranches` branches. `ratio` (default `0.5`) sets the inner/outer radius ratio (lower = more pointed).
|.**Triangle** ( cx : `Real` ; cy : `Real` ; r : `Real` {; rotation : `Real` } ) → `cs.svg` | Draws an equilateral triangle inscribed in the circle of center (`cx`,`cy`) and radius `r`, optionally rotated by `rotation` degrees.
...
|.**addElement** ( name : `Text` {; attributes : `Object` \| `Collection` } {; attachTo } ) → `cs.svg` | Creates an element of any `name` (with optional attributes) in the current container — an escape hatch for elements without a dedicated function.
|.**addTo** ( target : `Text` {; applyTo } ) → `cs.svg` | Attaches the element to the `target` container and resets the cursor to the root.
...
.**linearGradient** ( id ) : Object<br>
.**linearGradient** ( id; startColor ) : Object<br>
.**linearGradient** ( id; startColor; stopColor ) : Object<br>
.**linearGradient** ( id; startColor; stopColor; options ) : Object<br>
